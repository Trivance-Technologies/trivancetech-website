...
import { ArticleCard } from "@/libs/articles"; // import type
    ...
    <section className="w-full py-[6.25rem] px-[1rem] 1sm:px-[1.5rem] flex flex-col items-center bg-secondary">
      {isInititalDataLoading ? (
        <div className="flex justify-center items-center w-full">
          <Lottie animationData={loading} loop className="w-[200px] h-[200px] 2sm:w-[300px] 2sm:h-[300px]" />
        </div>
        ...
        <div className="flex flex-col mx-auto w-full max-w-[67.25rem] items-center">
          <motion.h2
            ...
            className="self-start uppercase tracking-[2px] font-medium text-[.875rem]/[1.25rem] 3sm:text-[1rem]/[1.25rem] opacity-[.6] text-primary mb-[4rem]"
          >
          ...
          >
            {tagList.map((tag, index) => (
              <button
                key={index}
                className={`px-4 py-2 hover:cursor-pointer text-sm font-medium text-gray-700 uppercase tracking-wide hover:bg-gray-100 ${
                  searchTag === tag ? 'border border-gray-400' : ''
                }`}
          ...
          {isArticlesDataFetching ? (
            ...
            >
              {articleCardData.length > 0 ? (
                ...
                <p className="col-span-3 text-center text-gray-500 py-10">No posts found matching your search.</p>
              )}
            </motion.div>
          )}
          {hasMore[searchTag] && (
        ...
        <div className="flex mx-auto w-full max-w-[67.25rem] items-center justify-center">
          <p className="col-span-3 text-center text-gray-500 py-10">There are currently no articles to display now</p>
        </div>
      )}
    </section>
  );
